...
// Idempotency guard — this file is mounted via <Script strategy="afterInteractive">
// from a Next.js layout. App Router soft navigations don't reload scripts, so we
// only ever want to run this once per session.
if (!window.__senscodeLegacyRan) {
window.__senscodeLegacyRan = true;
...
const runLegacyInit = () => {
  // Features below have been re-implemented as React components.
  // Listing them here short-circuits the original logic so we don't double-bind
  // listeners or fight React's state.
  const DISABLED_FEATURES = new Set([
    // Already re-implemented as React components.
    "lenis-scroll",
    "theme-toggle",
    "scroll-reveal",
    "back-to-top",
    ...
    "estimator",
    "nav-highlight",
    // These mutate React-rendered DOM in ways that crash the reconciler.
    // footer-curtain re-parents every <body> child into a wrapper div.
    // split-text-headings and typewriter wipe textContent on h1/h2 and inject
    // <span> children React didn't render — when React later unmounts those
    // headings (route change, theme change), it tries to remove text nodes
    // that no longer exist and throws "Failed to execute 'removeChild'".
    // Re-implement as React components in a follow-up pass to bring back.
    "footer-curtain",
    "split-text-headings",
    "typewriter",
    // card-tilt only ran once on initial page load — Next's soft nav meant new
    // cards never got the handler. Re-implemented as a React component that
    // re-binds on every pathname change. See components/CardTilt.tsx.
    "card-tilt",
  ...
  // downstream features still get a chance to run.
  const safe = (name, fn) => {
    if (DISABLED_FEATURES.has(name)) return;
    try {
      fn();
    } catch (err) {
  ...
  safe("intro-splash", () => {
    if (intro instanceof HTMLDialogElement) {
      if (
        ...
        window.addEventListener("wheel", introPreventScroll, {
          passive: false,
        });
        ...
        if (introCanvas) {
          ...
          const initIntroParticles = () => {
            ...
            const count = Math.min(
              ...
              Math.floor(
                (w * h * (1 + dpr)) / SITE_SETTINGS.introDensityDivisor
              )
            );
        ...
        intro.addEventListener("close", () => {
          ...
          // React owns this <dialog> node (rendered in app/page.tsx). Don't
          // physically remove it — closing the dialog already hides it via
          // the UA default stylesheet (`dialog:not([open]) { display: none }`).
          // Removing would crash React's reconciler on the next render.
        });
      } else {
        // Same reason — don't remove the React-owned node. The dialog stays
        // closed by default since we never called showModal() on it.
    ...
    if (footer) {
      if (!document.querySelector(".curtain-wrapper")) {
        ...
        while (
          ...
          document.body.firstChild !== footer
        ) {
  ...
  safe("custom-cursor", () => {
    if (window.matchMedia("(pointer: fine)").matches) {
      ...
      const localConstellation = document.getElementById(
  ...
  safe("reduce-motion", () => {
    const createA11yToggle = () => {
      const navActions = document.querySelectorAll(
      ...
      navActions.forEach((container) => {
        ...
        if (
          savedMotion === "true" ||
          (savedMotion === null && prefersReduced)
        ) {
...
// Boot: run immediately if the DOM is already past parsing (afterInteractive
// strategy usually lands here), else wait for DOMContentLoaded.
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", runLegacyInit);
} else {
  runLegacyInit();
}
